...
class _FakeRegistry:
    """Minimal registry: maps an agent name to a boundary that emits a fixed decision."""

    def __init__(self, agent: str, decision):
        self._agent = agent
...
def _decision(dtype="demo.act", source="agent", payload=None):
    return create_decision(id="d1", source=source, type=dtype, payload=payload or {})


def test_decision_level_checks_pass():
    reg = _FakeRegistry("k", _decision(dtype="demo.act", source="agent", payload={"x": 1}))
    case = EvalCase.from_dict({
        "name": "ok", "agent": "k",
        "expect": {"decision_count": 1, "decision_type": "demo.act", "source": "agent",
...
def test_wrong_kind_fails():
    reg = _FakeRegistry("k", _decision(dtype="demo.act"))
    case = EvalCase.from_dict({"name": "bad", "agent": "k", "expect": {"decision_type": "other"}})
    res = run_case(case, reg, execute=False)
    assert not res.passed
    assert any(c.name == "decision_type" and not c.ok for c in res.checks)


def test_unresolvable_kind_is_error():
    reg = _FakeRegistry("k", _decision())
    case = EvalCase.from_dict({"name": "missing", "agent": "nope", "expect": {"decision_count": 1}})
    res = run_case(case, reg, execute=False)
...
def test_result_contains_executes_handler():
    ...
    reg = _FakeRegistry("k", _decision(dtype="demo.act"))
    case = EvalCase.from_dict({"name": "exec", "agent": "k",
...
def test_deterministic_check_passes_for_pure_handler():
    ...
    reg = _FakeRegistry("k", _decision(dtype="act", payload={"n": 3}))
    case = EvalCase.from_dict({"name": "det", "agent": "k", "expect": {"deterministic": True}})
...
def test_deterministic_check_catches_flaky_handler():
    ...
    reg = _FakeRegistry("k", _decision(dtype="act"))
    case = EvalCase.from_dict({"name": "flaky", "agent": "k", "expect": {"deterministic": True}})
    res = run_case(case, reg, execute=True)
    assert not res.passed
...
def test_load_cases_skips_malformed(tmp_path: Path):
    ...
    (tmp_path / "evals" / "good.json").write_text(
        '{"name":"g","agent":"k","expect":{"decision_count":1}}', encoding="utf-8")
    (tmp_path / "evals" / "list.json").write_text(
        '[{"name":"a","agent":"k"},{"name":"b","agent":"k"}]', encoding="utf-8")
    (tmp_path / "evals" / "bad.json").write_text("{not json", encoding="utf-8")
    (tmp_path / "evals" / "noname.json").write_text('{"agent":"k"}', encoding="utf-8")
