...
def test_handler_runs_for_its_kind_with_collaborators():
    ...
    d = create_decision(id="d1", source="a", type="write", payload={"k": "v"})
...
def test_unknown_kind_refused_with_no_effect():
    ...
    d = create_decision(id="d1", source="a", type="mystery", payload={})
    result = apply(d)
...
def test_gate_rejected_decision_refused_and_no_handler_runs():
    ...
    d = Decision(id="", source="a", type="write", payload={})
    result = apply(d)
    assert result.applied is False
...
def test_custom_validator_rejection_blocks_handler():
    ...
    d = create_decision(id="d1", source="a", type="write", payload={})
    result = apply(d)
    assert result.applied is False
...
def test_on_accept_fires_before_handler_and_only_when_accepted():
    ...
    order.clear()
    rejected = Decision(id="", source="a", type="write", payload={})
...
def test_apply_is_deterministic_over_identical_decisions():
    ...
    d = create_decision(id="d1", source="a", type="write", payload={"n": 1})
