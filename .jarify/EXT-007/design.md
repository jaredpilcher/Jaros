...
```text
  ...
  loop (every tick, until SIGINT/SIGTERM):
        scan plugins/      -> import + register any new agent modules   (REQ-3)
...
```text
  <data>/
    ...
    plugins/      agent modules to load      (written by the CLI)
...
- Work and agents enter ONLY via `inbox/` and `plugins/` (shared FS) — no socket, no port.
